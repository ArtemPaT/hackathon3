<?php
	...
	if ($_POST["password"]!='' and $_POST["checkpassword"]!='' and $_POST["name"]!='' and $_POST["surname"]!='' and $_POST["login"]!='' and $_POST["email"]!='' and $_POST["phone"]!='') {
			...
			if ($table_count == null) {
				mysqli_query($con, "INSERT INTO accounts(name,surname,login,email,password,phone,ch) VALUES ('{$_POST['name']}','{$_POST['surname']}','{$_POST['login']}','{$_POST['email']}','{$_POST['password']}','{$_POST['phone']}',1)");
				...
				header("Location: regorg.php?che=155164");
			}
		}
		else {
			header("Location: regorg.php?id=112313241123");
		}
	}
	else {
		header("Location: regorg.php?lol=359089545809");
	}
